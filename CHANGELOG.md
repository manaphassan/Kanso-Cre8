...
### Added â€” Tactile Neumorphism (Soft UI) Physical Surface Engines
...
- **Standalone Windows Desktop Packaging & Version Synchronization**:
  - Synchronized workspace versioning across `package.json`, `tauri.conf.json`, `Program.cs` (`v0.2.2 â€¢ Zen Atelier`), and `build-windows.ps1`.
...
### Added â€” Priority 9 Commercial Engine & 100% Offline Perpetual Licensing
- **Zero-Database Offline Licensing Store (`licenseStore.svelte.ts`)**:
  ...
  - Commercial tier (*Kanso Studio Pro â€” $39 One-Time Perpetual*) unlocks business operations: unlimited client hub, dual-pane YAML invoice studio, print-ready PDF generation, billable chronometer rate counter, 5-folder project scaffolder, and deliverables packaging.
...
### Added â€” Priority 4 Bi-Directional Backlinks in Atelier Notes (`zettelService.ts`, `ZettelView.svelte`)
...
### Enhanced â€” Priority 8 Studio Deck & Dynamic Craft Metrics Bento (`DashboardView.svelte`)
...
### Added & Re-Architected â€” Studio & Freelance Brand Dossier
...
### Enhanced â€” Quotes & Invoices Studio (`InvoiceStudioView.svelte`, `financeService.ts`)
...
### Fixed & Refined â€” Retro Cassette Focus Radio (`CassetteDeck.svelte`)
- **Favorite Presets Counter Synchronization**:
  - Synchronized header presets badge (`FAVORITE STATIONS X PRESETS`) and tab counter (`â˜… FAV (X)`) with the actual active stations rendered via `$derived(validFavoriteStations.length)`.
...
## [0.0.1-alpha] - 2026-09-09 (Rebirth as Kanso Cre8 â€” Initial Alpha Release)

### Added & Rebuilt â€” Rebirth as Kanso Cre8 (ç°¡ç´ )
- **Product Identity & Creative Vault Philosophy**:
  - Rebranded the platform to **Kanso Cre8** (ç°¡ç´ ) â€” *The Mindful Creative Vault*.
...
### Added & Refined â€” Zero-Latency Navigation, Canva Cloud Bridge & Ecosystem Synchronization
...
- **Cross-Platform Profile Picture Auto-Sync (`UserProfileModels.cs`, `UserProfileService.cs`, `SettingsPage.xaml.cs`, `MainWindow.xaml.cs`)**:
  - Implemented automatic Base64-to-disk decoding and caching (`avatar_{staffId}.jpg`) from Synology NAS `staff_directory.json` to Desktop `%LOCALAPPDATA%\AtelierStudio\`.
...
- **Visual Gantt Timeline Off-Day Shading, Day Labels & Conflict Prevention (`CalendarPage.xaml`, `CalendarPage.xaml.cs`, `CategoryPresetService.cs`, `MalaysiaHolidayService.cs`, `ProjectGanttView.svelte`)**:
  ...
  - Color-coded badges and styling for Today (brand blue), Sundays (coral/red accent), Saturdays (slate neutral), and Malaysia Public Holidays (bold red with flag ðŸ‡²ðŸ‡¾).
  ...
  - Added active Gantt deadline conflict prevention: projects whose deadlines fall on an off-day display a prominent `âš ï¸ Off-Day` badge on the schedule bar, red warning border, red subtitle warning, and rescheduling guidance in tooltip.
...
### Added & Refined â€” NAS Attachment Vault, Task Handover, Multi-File Upload & Auto-Ingestion
...
- **Order Details Drawer & Attachment Actions (`OrderFormView.svelte`)**:
  ...
  - ðŸ‘ï¸ Preview / open media in browser, â¬‡ï¸ Download original file, and âŒ Safe delete.
  - ðŸ“‹ **"Copy NAS Folder Path"**: Copies `\\SSNAS\Creative-Team\_Orders\<ORDER_ID>` directly to clipboard for opening in Windows File Explorer.
  - âž• Upload additional reference files into the order vault at any time.
  - ðŸ“¥ **"Copy All Attachments to Project (01_BRIEF_ASSETS)"**: 1-click action to copy all staged assets into the linked project's brief folder on the NAS.
- **Desktop Project Creator Order Discovery & Ingestion (`ProjectCreatorPage.xaml`, `ProjectCreatorPage.xaml.cs`, `CreativeOrderService.cs`)**:
  - Added `LinkedOrderComboBox` to the desktop Project Creator page, automatically discovering pending orders from `_Orders/creative-orders.jsonl` with live attachment counts (`[ðŸ“Ž N files]`).
...
### Added & Refined â€” Ecosystem-Wide Creative Orders Real-Time Sync, Scaffolding Engine & Stability
- **Cross-Platform Creative Orders Synchronization (`CreativeOrderService.cs`, `OrderRequestsPage.xaml.cs`)**:
  - Direct live REST API integration between Desktop (Windows WPF & Linux Avalonia) and the Web Management Portal (`https://creative.AtelierStudio.myds.me/api/orders`).
...
- **Desktop Startup Sequence & Splash Screen Hang Resolution (`App.xaml`, `App.xaml.cs`, `MainWindow.xaml`)**:
  ...
  - Added robust diagnostic file logging (`%LOCALAPPDATA%\AtelierStudio\startup_trace.log`) and individual `try/catch` safety guards across all `MainWindow.OnLoaded` initialization routines.
...
## [4.6.0] - 2026-09-01 (Beta Release â€” Preflight Quality Auditor, Android Bento Telemetry, Persistent Caching, Live Radio Metadata & Desk Standby Mode)

### Added & Refined â€” Desktop Preflight Quality Auditor, Android 2Ã—2 Bento Telemetry, Persistent Caching & Live Radio Metadata Engine
...
- **Android 2Ã—2 Bento KPI Telemetry & Zero-Scroll Dashboard (`DashboardCompanionScreen.kt`)**:
  - Replaced horizontal scroll row with a structured 2Ã—2 Bento Grid (`Active Tasks`, `Due Projects`, `Active Brands`, `NAS Assets`).
...
- **Real-Time Live Radio Metadata & Broadcasting Engine (`LiveStreamMetadataFetcher`, `StudioRadioManager`)**:
  - Direct live API integration with **AtelierStudio AzuraCast** (`https://dj.AtelierStudio.myds.me/api/nowplaying/AtelierStudio-radio`) resolving live artist, track title, and album metadata on-air.
...
- **Interactive Ss-Hero Background Banner (`SsHeroSplashScreen.kt`)**:
  - Dynamic interactive particle wave mesh background canvas with ambient glow, floating geometry, and AtelierStudio brand symbols.
...
- **Adaptive Monochromatic Iconography (`ic_launcher_monochrome.xml`, `ic_AtelierStudio_mark_*.xml`)**:
  - Full Android 13+ Material You themed app icon support with vector AtelierStudio logomarks.

## [4.5.1] - 2026-08-30 (Cross-Platform Ecosystem Synchronization & Companion Harmonization)

### Synchronized & Unified â€” Desktop Client, Web Portal Manager Cockpit & Android Companion
...
### Added & Refined â€” Master Brand System v3.5.1, Web Portal Zero-Emoji Overhaul, Multi-Format Color Matrix & 5 Sub-Brands Hub
- **Web Portal Brand System & Zero-Emoji Modernization (`SS-CAM.Web`)**:
  ...
  - Built canonical [`FluentIcons.svelte`](file:///d:/HaNa_Innovation/ss_cam/src/SS-CAM.Web/client/src/lib/components/ui/FluentIcons.svelte) providing 45+ type-safe Microsoft Fluent 2 SVG icons styled in the official AtelierStudio brand palette.
...
- **Master Brand System Architecture Alignment (`BrandAssetsPage.xaml`, `BrandAssetsPage.xaml.cs`)**:
  - Full synchronization with the authoritative [AtelierStudioâ„¢ Master Brand System Guide](https://assets.AtelierStudio.myds.me/brand-system/).
  - Implemented the Two-Layer System Architecture: AtelierStudio Identity Visual Layer combined with Fluent 2 Dynamic Interaction Logic.
...
- **5 Operating Corporate Sub-Brands Hub (`BrandAssetsPage.xaml`, `BrandAssetsPage.xaml.cs`)**:
  - Dedicated identity cards with visual badges, operational scope descriptions, and 1-click local Explorer folder launchers for:
    - **SS Health** (`01_logo_ssHealth`) â€” Parent holding company, governance & legal
    - **SS Clinic** (`02_logo_ssClinic`) â€” Clinical men's health consultations & medical care
    - **SS Wellness** (`03_logo_ssWellness`) â€” Physiotherapy, vitality recovery & lifestyle care
    - **SS Ecommerce** (`04_logo_ssEcom`) â€” Official store vectors, packaging & digital retail
    - **SS Technology** (`05_logo_ssTech`) â€” Digital health apps, EHR integrations & developer graphics
- **Master Logo & Surface Contrast Previewer (`BrandAssetsPage.xaml`, `BrandAssetsPage.xaml.cs`)**:
  ...
  - Direct "Open Master Kit" folder launcher for `00_logo_AtelierStudio`.
- **4-Tier Typography Scale Reference (`BrandAssetsPage.xaml`)**:
  - Visual hierarchy cards detailing standard weights and usage roles for:
    - **Poppins** (Display & Hero â€¢ 600, 700, 800)
    - **Montserrat** (Subtitles & UI â€¢ 500, 600, 700)
    - **Helvetica Neue** (Body & Web â€¢ 400, 500, 700)
    - **Calibri** (Office & Email â€¢ 400, 700)
- **Workstation Asset Deployment Hub (`BrandAssetsPage.xaml`, `PayloadInstallerService.cs`)**:
  - One-click font registration, vector asset local deployment (`%LOCALAPPDATA%\AtelierStudio\Assets`), and desktop shortcuts creation.
...
### Added & Refined â€” Real-Time Biometric Radar, 30-Day Heatmap & Hydration Tracker
...
### Added & Refined â€” Dynamic Hero Audio Visualizer, Playback Gating & Curated Stations
- **Dynamic Real-Time Playback Gating (`RadioPage.xaml`, `RadioPage.xaml.cs`)**:
  - The hero backdrop scattered Mars symbols (`â™‚`) and AtelierStudio logomarks are completely hidden (`Opacity = 0.0`) when radio is stopped or paused, seamlessly fading in on active playback.
...
### Added & Refined â€” Radio Stream Integration, Deep Scanner Routing & Copywriting FlowDocument Engine
- **Official AtelierStudio Radio Stream Integration (`RadioStreamService.cs`)**:
  - Integrated official AtelierStudio Radio Stream (`https://dj.AtelierStudio.myds.me/listen/AtelierStudio-radio/radio.mp3`) as pinned `#1` preset station with live audio playback.
...
### Fixed â€” Dashboard, Role Filtering & Scroll
- **Dashboard Designer Workload â€” Year Bug (`WorkloadSlaService.cs`)**:
...
- **Mouse Wheel Scrolling Restored Across All Pages**:
  ...
  - Added global `OnGlobalPreviewMouseWheel` handler on `MainWindow` `NavigationView` â€” walks visual tree to find nearest scrollable `ScrollViewer`, performs step-clamped `LineUp()`/`LineDown()`/`LineLeft()`/`LineRight()`. Covers all pages automatically.
...
### Added & Refined â€” Designer Workload Heatmaps & Creative SLA Analytics
...
### Added & Refined â€” Asset Export, Packaging & Naming Engine
...
- **Canonical Asset Naming & Sanitizer (`AssetNamingService.cs`)**:
  - Enforces canonical AtelierStudio standard `{YEARMONTH}_{JOBID}_{BRAND}_{PROJECT}_{TYPE}_{VERSION}.{EXT}`.
...
### Added & Refined â€” Desktop NAS Sync & Batch Operations + Web Real-Time SSE
...
### Added & Refined â€” Desktop Feature Parity & Studio Overhaul
...
### Added & Refined â€” Centralized Studio Hierarchy, ClickUp Task Workspace & Copywriting Studio
...
- **Official AtelierStudio Brand Guidelines & Strict Typography Synchronization**:
...
### Added & Refined â€” Metamorphosis Theme Legibility & Surface Opacity Overhaul
...
### Added & Refined â€” Fluent UI Web Design System & Data Visualization Analytics Release
...
### Added & Refined â€” Designer Companion & In-App Brief Editor Release
...
- **Task Manager FIFO Queue & Date Filter Upgrades (`TaskManagerPage`)**:
  - Added `created:` frontmatter tag support, queue age display (`ðŸ“… 14d in queue`), FIFO Queue Order sorting (`Oldest First`), and `Created Date` drawer editor.
...
### Added & Refined â€” Feature Release
...
### Added & Refined â€” Feature Release
- **Big Calendar Module (`CalendarPage`)**: Native 7Ã—6 monthly calendar timetable displaying project creation start dates and campaign deadlines as color-coded chips, Friday Solat indicators, interactive Day Detail Overlay inspector, month switcher navigation (`â—€ Prev`, `Today`, `Next â–¶`), and real-time search & designer filters.
- **Task Manager Queue Management & Calendar Dates**:
  ...
  - `AgeDisplay` & `AgeBadgeColor`: Visual queue age indicators on project cards (e.g. `ðŸ“… 14d in queue`, color-coded amber for `>30d` and red for `>60d`).
...
- **Sound Engineer Studio Visualizer & Mars Crest FX**: Upgraded `VisualizerService` with scattered floating Mars symbols (â™‚) and AtelierStudio brand crest, custom canvas particle physics, audio peak reactive motion, and clean canvas removal of text watermarks.
...
- **Designer Profile & Settings Revamp**: Rebuilt `SettingsPage` with 2-column layout, section icon badges, interactive theme swatches (Falconia, Metamorphosis, Catppuccin, RosÃ© Pine, Nord), workstation payload installer, category preset management, and reset/maintenance action rows.
...
### Added & Refined
- **Header Title Rebranding & TitleBar Overhaul**: Rebranded app header title to `SS Creative Assets Management`. Applied a 100% full-width `#022057` AtelierStudio deep blue TitleBar with transparent caption buttons (`â€” ðŸ—– âœ•`) and window dragging capability (`DragMove`).
- **Persistent SS Blue Bottom Player Bar**:
  - Styled bottom player with full AtelierStudio deep blue background (`#022057`) and `#043388` accent border.
  - **Fluid Wavelength Audio Visualizer**: Integrated a 60 FPS real-time vector path visualizer (`StreamGeometry`) featuring a 4-stop gradient stroke (`#60A5FA` â†’ `#38BDF8` â†’ `#34D399` â†’ `#818CF8`) and ambient gradient fill under the wave curve.
...
### Added â€” Phase 1: High-Impact UI/UX Modernization
...
### Added â€” Phase 2: High-Impact Workflow Automation
...
### Added â€” Phase 3: Visual Tools & Assets Management
- **Global Brand Kit Quick-Tray**: Added `ðŸŽ¨ Brand Kit` quick button to title bar with a popover tray for 1-click HEX color swatch clipboard copying application-wide.
- **Visual Asset Lightbox**: Upgraded image previewer in Search & Copy to a high-definition dark Fluent 2 Lightbox modal backdrop (`#0B1120`) displaying pixel dimensions, file size, format badges, and action controls (`ðŸ“‹ Copy Path`, `âš¡ Open File`).
...
- **`FrontmatterService.ParseFrontmatter`** â€” Parser discarded all successfully-parsed key-value pairs when a README.md was missing its closing `---` delimiter (fell through to `return null`). Changed to `return result` so partially-formed frontmatter is still honoured. Affected Task Manager column placement for any project with a malformed README.

### Fixed â€” High (P1)

- **Theme toggle** â€” `OnStatusThemeToggle` event handler existed but had no XAML binding. Added `MouseLeftButtonDown` and `Cursor="Hand"` to the Theme status row in the sidebar footer with a `ToolTip` describing the cycle order (SS Default â†’ Falconia â†’ Metamorphosis).
- **Orphaned handlers removed** â€” `OnOpenGithub` and `OnOpenAboutWindow` were dead code-behind methods with no XAML targets. Removed to reduce maintenance surface.
- **Dead sidebar-collapse fields removed** â€” `isSidebarExpanded` (bool) and `_lastActiveNavBtn` (Button) were written but never read. These were remnants of the pre-WPF-UI custom sidebar. Removed; compiler warnings eliminated.

### Fixed â€” Medium (P2)

- **Hardcoded `D:\Testing` workspace default** â€” `DashboardPage`, `SearchCopyPage`, and `ProjectCreatorPage` all used `D:\Testing` as the initial `workspaceRoot` value. Changed to `string.Empty` so unconfigured installs show an empty/graceful state instead of silently scanning a non-existent path.
- **Team Board offline write** â€” `TeamBoardService.GetNotesPath` previously created the `_Team` folder on whatever path was provided, including local fallbacks. Added an early guard (`Directory.Exists(workspaceRoot)`) that returns `null` for inaccessible roots. `Save()` now checks for a `null` path and returns `false` immediately, propagating the correct error to the UI.
- **Workstation Health rescan count** â€” `OnRescanSoftwareClicked` showed a hardcoded "11 design software packages" confirmation regardless of actual scan results. Refactored to call `ScanInstalledDesignSoftware()` once and use the real count with proper pluralisation.
- **Static `HttpClient`** â€” `FetchDesignArticlesAsync` created a new `HttpClient` instance per fetch. Replaced with a `private static readonly HttpClient _httpClient` singleton on `DashboardPage` to prevent socket exhaustion. `UserAgent` is now cleared and re-set on each call.
- **Quick Notes H3 preview** â€” `RenderPreview` handled `# H1` and `## H2` but had no branch for `### H3`. Added `###` case with `FontSize=14, FontWeight=SemiBold`. Corrected check order to `### â†’ ## â†’ #` to prevent prefix collision.

### Fixed â€” Low (P3)

- **Silent `catch {}` blocks** â€” `FrontmatterService` (ReadStatus, WriteStatus) and `TeamBoardService` (LoadNotes, Save, GetNotesPath) now log exceptions via `System.Diagnostics.Debug.WriteLine` with `[ServiceName]` prefixes instead of silently discarding errors. Aligns with AGENTS.md error-handling rules.
...
### Added â€” Quick Notes Module
- **`QuickNotePage`** â€” Full-page Markdown note editor backed by `QuickNoteService`. Notes are saved as `.md` files in `%APPDATA%\SS-CAM\notes\` with automatic 3-second debounce via `DispatcherTimer`.
- **Sidebar nav item** (ðŸ“ Quick Notes) between Radio Player and Workstation Health.

### Added â€” Task Manager Module
- **`TaskManagerPage`** â€” Kanban-style board reading project status from `README.md` YAML frontmatter via `FrontmatterService`. Columns: Not Started Â· In Progress Â· Review Â· Done Â· On Hold.
- **Inline drawer** â€” Clicking a project card opens a slide-in editor with status, priority, deadline, and brief fields that writes back to frontmatter on save.
- **Sidebar nav item** (ðŸ—‚ Task Manager) between Quick Notes and Workstation Health.

### Added â€” Team Board (Dashboard)
- **Team Board card** on the Dashboard page â€” displays last 10 shared notes from `_Team/team-notes.json` on NAS, with Author, timestamp, pin toggle, and delete actions.
- **Post Note input row** â€” Post a message visible to all team members; falls back gracefully when NAS is offline.
- **30-second auto-refresh** via `DispatcherTimer` that starts/stops cleanly with page navigation lifecycle.

### Added â€” Services
- **`QuickNoteService`** â€” Load/save personal Markdown notes to `%APPDATA%\SS-CAM\notes\`.
- **`FrontmatterService`** â€” Read and write YAML frontmatter blocks (`--- ... ---`) in any Markdown file without disturbing body content. Includes `BuildDefaultFrontmatter()` for new projects.
- **`TeamBoardService`** â€” Load, post, pin/unpin, and delete team notes via a shared `_Team/team-notes.json` file on the NAS workspace root.

### Changed â€” Project Creator: Frontmatter Injection
- **README.md now includes YAML frontmatter** â€” Every newly created project folder generates a `README.md` prefixed with a default frontmatter block (`status`, `priority`, `designer`, `brand`, `deadline`, `revision`, `tags`) so the Task Manager can index it from day one.
- Sub-brand code is extracted from the ComboBox selection and written into the `brand:` field automatically.

### Changed â€” Navigation
- **MainWindow nav** â€” Two new nav buttons registered and routed: Quick Notes (icon `&#xE70B;`) and Task Manager (icon `&#xE9D5;`).

### Changed â€” Version
- Version badge bumped from `v2.3.6` â†’ `v2.5.0` in `DashboardPage.xaml` and window title.
...
### Added â€” Dashboard: Designer Inspiration Widget
- **Designer Insight card** â€” Full-width widget below the metric tiles showing a rotating pool of 40 curated design tips (colour theory, print production, typography, file hygiene, brand discipline, creative wellbeing, and more).
- **Auto-advance timer** â€” Tips rotate automatically every 60 seconds via a `DispatcherTimer` that is cleanly stopped on page unload.
- **Next Tip button** â€” Manually advance to the next tip at any time.
- **Smashing Magazine RSS feed** â€” Toggle the "Articles" button to fetch and display the 5 latest design articles from Smashing Magazine as clickable links. Falls back silently to offline tip pool if the network is unavailable or times out (6-second threshold).

### Changed â€” Project Creator: Three Refinements
- **Editable canvas extension** â€” `TemplateExtensionComboBox` is now editable (`IsEditable="True"`), allowing designers to type any custom file extension (e.g. `.indd`, `.sketch`, `.fla`). The live directory tree preview and the generated file on disk reflect the typed value immediately.
- **Project Brief â†’ Markdown Editor** â€” Replaced the 64px `ui:TextBox` for *Project Brief / Remarks* with a taller 200px scrollable `TextBox` preceded by a 6-button **Markdown toolbar**: Bold (`**`), Italic (`*`), Inline Code (`` ` ``), H2 Heading (`##`), List Item (`-`), and Horizontal Rule (`---`). Toolbar buttons wrap or prefix selected text; if no text is selected they insert a placeholder. The richer Markdown content is written verbatim into `README.md` on folder creation.
- **Checkbox labels cleaned** â€” Removed numeric folder-prefix noise from checkbox labels: `Include 05_Revisions folder` â†’ `Include Client Revisions folder`, `Include 06_Raw_Media folder` â†’ `Include Raw Media folder`. The created folder names on disk are unchanged.

### Changed â€” Search & Copy: Catalog Book Layout
- **Catalog book layout** â€” Completely redesigned from a wide DataGrid + narrow sidebar to a **270px fixed sidebar + dominant README pane**: the right README preview now fills all available vertical height using `DockPanel.LastChildFill` rather than a fixed `Height="360"` DataGrid.
- **Project sidebar cards** â€” Replaced the `DataGrid` with a styled `ListBox` of folder cards. Each card shows a folder icon glyph, project name, and a compact metadata line (`files Â· size Â· modified date`). Selected card is highlighted with `FluentBrandLight` background and `FluentBrandTint` border.
- **Mode toggle strip** â€” PREVIEW / Raw / Gallery toggle buttons moved from below the search bar into the selected project header badge for immediate access without scrolling.
- **Action buttons docked to footer** â€” Copy Path and Copy Whole Project Folder buttons repositioned to a docked footer panel at the bottom of the right pane, keeping the README preview area unobstructed.
- **Live project count** â€” The sidebar header now shows a live count label (e.g. "42 projects") that updates after every search.
...
- **Token-based colour system** â€” Removed all raw hex colour literals from `WellbeingPage.xaml`, `SearchCopyPage.xaml`, `RadioPage.xaml`. All foreground, background, border, and status colours now reference `FluentBrand80`, `FluentLightTextPrimary`, `FluentLightTextSecondary`, `FluentLightCardBg`, `FluentLightCardSubBg`, `FluentLightStroke`, `FluentBrandLight`, `FluentBrandTint`, `FluentDanger`, and `FluentSuccess` token brushes defined in `Styles/Fluent2Styles.xaml`.
- **Typography consistency** â€” Applied `{StaticResource FluentFontFamily}` to all TextBlocks that previously used hard-coded `Segoe UI Variable Text` or `Segoe UI Variable Display` font family strings.
- **Button spec** â€” All primary action buttons now use `FluentBrand80` background / White foreground with `BorderThickness="0"` and `Cursor="Hand"`. Secondary buttons use `FluentLightCardSubBg`. Danger-zone buttons use `FluentDanger`.
...
- **Radio & Focus Stream Player Module**:
  ...
  - **Status Bar Mini-Player Widget**: Persistent audio controls (`â–¶`/`â¸` play toggle, live station indicator) built directly into the bottom status bar, accessible from all pages.
  - **Interactive Station Grid & Visualizer**: Station cards with category filter tabs (`â­ Favorites`, `Focus`, `Pop/Hits`, `Talk/News`, `Jazz/Chill`, `Custom`) and animated EQ visualizer bars during active playback.
  - **Configuration Persistence**: Automatically saves custom streams, starred favorites, last played station, and volume preferences to `%LOCALAPPDATA%\AtelierStudio\radio_config.json`.
...
### Added
- **Creative Wellbeing Module** â€“ A completely local, private companion for designers to help maintain healthy work habits.
...
### Fixed
- **Window Icon** â€” Fixed an issue where the WPF Window taskbar icon and title bar were displaying as the default PowerShell logo by loading the `AtelierStudio-logo-on-dark-ui.png` directly as the WPF `Window.Icon` instead of relying on `.ico` format parsing.
...
### Fixed
- **App Icon** â€” Fixed an issue where the application icon was displaying as the default PowerShell logo by generating and embedding a proper `.ico` asset containing the AtelierStudio brand logo.
...
### Added
- **Collapsible sidebar** â€” Added a hamburger menu button in the header (top left) that allows the sidebar to be collapsed/expanded to maximize workspace area.

### Fixed
- **Circular avatar profile picture** â€” Fixed the avatar picture displaying as a square by updating the `Image` to a `Border` with a rounded `CornerRadius` using an `ImageBrush` background, making it perfectly circular.
...
- **App crash on launch** â€” Fixed a bug where the app would crash immediately upon opening because the animated header canvas was not in the script scope, causing the `DispatcherTimer` tick handler to fail.
...
- **Avatar click-to-preview** â€” clicking the circular profile photo in the sidebar now opens a full-size image popup (`860Ã—660` resizable dark window). The click is captured via `PreviewMouseLeftButtonDown` with `Handled=true` so it doesn't trigger the NavProfile navigation.
- **Department/role in sidebar** â€” the subtitle under the designer name now shows the department/role from User Profile settings (e.g. "Design Lead"), falling back to "User Profile" when empty. Implemented via a new `DepartmentDisplay` computed property on `AppViewModel` that reacts to `Department` changes.
- **Production / Export thumbnail tab** â€” a new third tab in the Search & Copy right panel scans `Production`, `Export`, `Exports`, `Output`, and `Outputs` subfolders of the selected project for image files (`.png .jpg .jpeg .gif .bmp .tiff .webp`). Results appear as `160Ã—120` thumbnail cards in a `WrapPanel`; clicking any card opens the full-size popup.
- **Animated header** â€” the dark navy header has a looping PS Vita-style animation of 8 semi-transparent floating circle outlines (`Ellipse`, white stroke, 5â€“9% opacity). Driven by a 33 ms `DispatcherTimer`; shapes wrap around canvas edges. Timer is stopped cleanly on `Window.Closed`.
...
- Release executable signed with AtelierStudio certificate, timestamped via DigiCert (2026-08-04).
...
- **Job ID code** no longer stays stuck on `D` when switching creative presets. Changing the preset now immediately updates the suffix letter in the Job ID field (e.g. `0003D` â†’ `0003S` for Social, `0003V` for Video, `0003P` for Brand Identity).
...
- Hardcoded fallback version strings in `Get-AtelierStudioLatestRelease` and `Build-Installer.ps1` bumped to `1.9.3` for consistency.

### Documentation

- Replaced outdated WinForms installer screenshots in `docs/` with fresh WPF v1.9.3 captures:
  - `app-dashboard.png` â€” Creative Workspace Dashboard
  - `app-project-creator.png` â€” Creative Project Folder Creator
  - `app-search-copy.png` â€” Search & Copy
  - `app-brand-assets.png` â€” Brand Assets module
  - `app-profile-settings.png` â€” User Profile & Settings
  - `app-installer-setup.png` â€” Installer Setup wizard
...
- Font Awesome Free vector icons and AtelierStudio favicon integration for the window, taskbar, and report dialogs.
...
- Release executable signed with AtelierStudio certificate (SHA1: `9A73B71BEE3AFEDA9E4CCECA2466FB5FFE0255AC`), timestamped via DigiCert on 2026-08-04.
