<script lang="ts">
  ...
  const QUICK_ACTIONS: QuickAction[] = [
    ...
    { type: 'action', id: 'nav-clients', label: 'Open Clients & Brands Hub', icon: 'users', category: 'Navigation (⌘4)', execute: () => appState.navigate('clients') },
    ...
    { type: 'action', id: 'nav-copy', label: 'Open Copywriting Studio', icon: 'sparkles', category: 'Studio (⌘7)', execute: () => appState.navigate('copy-studio') },
    { type: 'action', id: 'nav-radio', label: 'Open Focus Radio & Cassette Deck', icon: 'colorPalette', category: 'Audio (⌘8)', execute: () => appState.navigate('radio') },
